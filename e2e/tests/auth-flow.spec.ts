...
test.describe('Child selection and PIN flow', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // After login the app does navigate('/parent') — wait for that client-side navigation
    await page.waitForURL(/\/parent/, { timeout: 5000 });

    // Go back using browser history (client-side — no full reload, so parentToken stays in memory)
    await page.goBack();
    // Confirm we're on the lobby before each test runs
  ...
  test('clicking child card shows PIN modal with password input', async ({ page }) => {
    await page.getByText('דני').click();
